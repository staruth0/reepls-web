...
const PublicationNotificationContainer: React.FC<PublicationNotificationProps> = ({
  username,
  timestamp,
  communique,
  is_read,
  ...
  const updateStatus = () => {
    // Prioritize publication_id from backend, fallback to publicationId, then article_id
    const publicationIdValue = publication_id || publicationId;
    if (publicationIdValue) {
      navigate(`/stream/${publicationIdValue}`);
    } else if (article_id) {
      navigate(`/stream/${article_id}`);
    } else if (url) {
      // If url contains /publications/, convert to /stream/
      const convertedUrl = url.replace('/publications/', '/stream/');
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <div className="relative">
          <div className="absolute -left-2 top-1/2 -translate-y-1/2">
            <LuBookOpen className="w-4 h-4 text-blue-400/60" />
          </div>
...
export default PublicationNotificationContainer;
